...
4. Set the **Goal** and choose **Create roadmap & start**. AI builds the milestone graph and automatically delegates available steps to employees. Approving a deliverable unlocks its dependent work. You can also select an employee to give a manual assignment. Optional local file copies are shared only when explicitly selected and authorized.
...
- **Navigation:** Office, Employees, and Roadmap, in that order. Office fits the window without page scrolling; chat and long dialogs scroll internally. The top bar contains the date, Settings, and a profile button that opens a compact ChatGPT sign-in popup. Reviews and activity/export are also available inside Settings.
