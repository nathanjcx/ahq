...
import type { Channel, Memory, Post } from '@/lib/contracts';
...
const floorMemories: Memory[] = [
  {
    id: 'mem_f1',
    scope: 'floor',
    scopeId: 'proj_launch',
    kind: 'decision',
    text: 'Migration notes lead with what breaks for a customer; the schema table follows.',
    tags: ['copy', 'launch'],
    author: 'person',
    authorName: 'Dana Okoye',
    confidence: 1,
  ...
  },
  {
    ...
    scopeId: 'proj_launch',
    kind: 'glossary',
    text: '"Backfill" on this floor means the one-off job that populates the new columns, not the nightly sync.',
    tags: ['glossary'],
    author: 'janitor',
    authorName: 'Janitor',
    confidence: 0.9,
  ...
  },
  {
    ...
    scopeId: 'proj_launch',
    kind: 'procedure',
    text: 'Cite the test run in the changelog whenever a migration is described as tested.',
    tags: ['audit'],
    author: 'agent',
    authorName: 'Gil',
    confidence: 0.6,
  ...
  },
  {
    ...
    scopeId: 'proj_launch',
    kind: 'fact',
    text: 'The launch deadline is March 24.',
    tags: ['deadline'],
    author: 'agent',
    authorName: 'Ada',
    confidence: 0.5,
    status: 'contested',
    contestedWithId: 'mem_f5',
    contestReason: 'Two shift reports on the same day cite different dates, and neither names a source.',
    createdAt: ago(22),
    updatedAt: ago(2.5),
  },
];
...
export const channelsQueries: FixtureQueries = {
  ...
  // The Records room owns this answer; the floor binder only needs the floor scope of it.
  'memory:list': (args: unknown) => {
    const { scope, scopeId } = args as { scope?: string; scopeId?: string };
    return scope === 'floor' && scopeId === 'proj_launch' ? floorMemories : [];
  },
};
