...
export const employeesQueries: FixtureQueries = {
  ...
  'memory:list': (args: { scope?: string; scopeId?: string }) =>
    memories.filter(
      (entry) =>
        (!args.scope || entry.scope === args.scope) &&
        (!args.scopeId || entry.scopeId === args.scopeId),
    ),
  'audit:findings': (args: { employeeId?: string }) =>
    findings.filter((finding) => !args.employeeId || finding.employeeId === args.employeeId),
  'channels:list': [
    { id: 'chan_launch', kind: 'floor', scopeId: 'proj_launch', name: 'Spring launch', unread: 2 },
