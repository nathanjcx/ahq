...
import { agendaDays } from './rows';
import type { CalendarEntry, CalendarKind } from '@/lib/contracts';

const HEADING = new Intl.DateTimeFormat(undefined, { weekday: 'long', month: 'short', day: 'numeric' });
    ...
    <div className="cal-agenda card">
      {agendaDays(entries, bounds).map((day) => (
        <section key={day.start}>
          <h3 data-today={new Date(day.start).toDateString() === today}>
            {HEADING.format(day.start)}
            {new Date(day.start).toDateString() === today && <span>Today</span>}
          </h3>
          {day.entries.length ? (
