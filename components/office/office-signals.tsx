...
import { Box, Round, type Point } from './office-primitives';
  ...
  return (
    <group position={position}>
      ...
      >
        ...
        <Box p={[0, 0.96, 0.03]} s={[0.48, 0.06, 0.3]} color={C.seatSoft} />
        {/* The reading surface slopes down toward a reader standing at +z. */}
        <group position={[0, 1.02, 0.01]} rotation={[0.5, 0, 0]}>
          <Round p={[0, 0, -0.02]} s={[0.58, 0.05, 0.46]} color={C.wood} radius={0.02} />
          <Box p={[0, 0.055, 0.2]} s={[0.58, 0.1, 0.035]} color={C.seatSoft} />
          <group position={[0, 0.04, -0.12]}>
            ...
            <Box p={[-0.2, 0.035, 0]} s={[0.02, 0.07, 0.26]} color={C.seatSoft} />
            <Box p={[0.2, 0.035, 0]} s={[0.02, 0.07, 0.26]} color={C.seatSoft} />
            <Box p={[0, 0.035, -0.13]} s={[0.42, 0.07, 0.02]} color={C.seatSoft} />
            {[0, 1, 2].map((sheet) => (
              <Box
                key={sheet}
                p={[(sheet - 1) * 0.015, 0.026 + sheet * 0.008, (sheet - 1) * 0.01]}
                s={[0.33, 0.006, 0.23]}
                color={[C.paper, C.paperShade, C.paperDim][sheet]}
      ...
      <pointLight ref={lamp} position={[0, 1.42, 0]} color={C.lampHalo} intensity={0} distance={2.4} />
      {glowing && (
        <Html center position={[0, 1.36, 0]} zIndexRange={[18, 8]}>
          <button
            ...
            style={{ '--tray': stuck ? C.red : C.amber } as CSSProperties}
            title={
...
export function ProviderConsole({
  ...
  useLayoutEffect(() => {
    bars.color.set(degraded ? C.degraded : color);
  }, [bars, color, degraded]);
  useFrame((state) => {
    if (!motion) return;
    ...
    if (degraded) {
      ...
      bars.color.set(sputter > 1.15 ? C.beacon : C.degradedDim);
    } else {
  ...
  return (
    <group position={position} name={name}>
      {[-1, 1].map((side) => (
        <Box key={side} p={[side * 0.24, 0.035, 0.04]} s={[0.1, 0.07, 0.32]} color={C.metalDark} />
      ))}
      <Round p={[0, 0.33, 0]} s={[0.66, 0.52, 0.44]} color={C.cabinet} radius={0.05} />
      <Box p={[0, 0.69, -0.05]} s={[0.4, 0.3, 0.24]} color={C.cabinetDeep} />
      {/* The screen leans toward the room, tinted with the provider colour. */}
      <group position={[0, 0.85, 0.02]} rotation={[0.55, 0, 0]}>
        <Round s={[0.56, 0.34, 0.03]} color={C.screenFrame} radius={0.02} />
      ...
      <mesh position={[0, 0.22, 0.222]}>
        ...
        <meshStandardMaterial color={C.screen} emissive={color} emissiveIntensity={0.8} toneMapped={false} />
      </mesh>
    </group>
  );
}
...
export function StatusDevice({
  ...
  useFrame((state) => {
    ...
    material.color.set(bright ? C.amberLit : C.degradedDim);
    material.emissiveIntensity = bright ? 1.05 : 0.05;
  });
  return (
    <group position={position}>
      <Round s={[0.3, 0.34, 0.11]} color={C.cabinet} radius={0.045} />
      <Box p={[0, -0.2, 0]} s={[0.12, 0.06, 0.09]} color={C.screenFrame} />
      ...
      <mesh position={[0, 0, 0.058]} rotation={[Math.PI / 2, 0, 0]}>
        ...
        <meshStandardMaterial color={C.screenFrame} roughness={0.6} />
      </mesh>
      <mesh position={[0, 0, 0.07]} rotation={[Math.PI / 2, 0, 0]}>
        <cylinderGeometry args={[0.062, 0.062, 0.02, 20]} />
        <meshStandardMaterial
          ref={lens}
          color={degraded ? C.amberLit : C.working}
          emissive={degraded ? C.amberLit : C.working}
      ...
      {seats.map((seat, index) => {
        ...
        return (
          <group
            ...
            userData={{ speaking }}
          >
            <Round s={[0.66, 0.46, 0.07]} color={speaking ? C.amber : C.paper} radius={0.09} />
            {/* The tail, pointing down at whoever is speaking. */}
            <Box
              p={[-0.16, -0.26, 0]}
              s={[0.16, 0.2, 0.06]}
              color={speaking ? C.amber : C.paper}
              rotation={[0, 0, 0.5]}
            />
            {[-0.13, 0.08].map((x) => (
              <group key={x} position={[x, 0.04, 0.04]}>
                <Box s={[0.066, 0.14, 0.012]} color={C.ink} />
                <Box p={[-0.02, -0.12, 0]} s={[0.038, 0.11, 0.012]} color={C.ink} />
              </group>
            ))}
          </group>
        );
