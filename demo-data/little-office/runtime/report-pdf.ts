...
export async function writeReportPdf(file: string, title: string, markdown: string): Promise<void> {
  const doc = new PDFDocument({ size: 'A4', margin: 48, info: { Title: title, Author: 'Little Office' } });
  const saved = pipeline(doc, createWriteStream(file, { flags: 'wx' }));
  const plain = (text: string) => text.replace(/→/g, '->').replace(/←/g, '<-').replace(/↓/g, ' (lower)').replace(/↑/g, ' (higher)').replace(/−/g, '-').replace(/\*\*|__|`/g, '').replace(/\[([^\]]+)\]\(([^)]+)\)/g, '$1 ($2)');
  doc.font('Helvetica-Bold').fontSize(10).fillColor('#52706b').text('LITTLE OFFICE / REPORT');
  doc.moveDown();
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    if (line.trim().startsWith('|')) {
      ...
      doc.table({ data: rows, defaultStyle: { padding: 5, border: 0.5, borderColor: '#d9d5c7' }, rowStyles: row => row === 0 ? { backgroundColor: '#e7eee7' } : {} });
      doc.moveDown(0.5);
      continue;
    }
    ...
    doc.font(heading ? 'Helvetica-Bold' : 'Helvetica').fontSize(heading ? heading[1].length === 1 ? 20 : 13 : 10)
