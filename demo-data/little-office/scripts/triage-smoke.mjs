...
// Uses real Codex triage and task execution, consuming subscription allowance.
...
try {
  ...
  const chosen = process.env.OFFICE_TRIAGE_EVENTS?.split(',');
  const events = chosen ? chosen.map((id) => { const event = state.demo.events.find((item) => item.id === id); assert.ok(event, `Unknown event ${id}`); return event; }) : state.demo.events.slice(0, limit);
  assert.ok(events.length > 0);
  await mkdir('test-results', { recursive: true });
  for (const event of events) {
    const priorSources = new Set(state.sources.map((source) => source.id));
    console.log(`Delivering ${event.id}: ${event.label}`);
    state = await command({ type: 'demo.deliver', id: event.id });
    const incoming = state.sources.find((source) => !priorSources.has(source.id));
    if (!incoming) {
      results.push({ event: event.id, duplicate: true });
      continue;
    }
    const record = state.triage.find((item) => item.sourceId === incoming.id);
    assert.ok(record, "Every new arrival needs a persisted triage record.");
  ...
  do {
    await new Promise((resolve) => setTimeout(resolve, 500));
    state = await command({ type: 'snapshot' });
    if (!state.work.some((work) => ['running', 'queued'].includes(work.status)) && !state.triage.some((item) => ['running', 'queued'].includes(item.status))) break;
  } while (Date.now() < deadline);
  assert.ok(!state.work.some((work) => ['running', 'queued'].includes(work.status)), 'All ready work must finish.');
  assert.deepEqual(state.work.filter((work) => work.status === 'failed').map((work) => ({ title: work.title, error: work.error })), [], 'No execution should fail.');
  assert.equal(state.triage.filter((item) => item.status === 'failed').length, 0, 'All intake decisions must succeed.');
