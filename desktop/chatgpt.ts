  ...
  private async startSession(
    ...
    this.launch(
      ...
      JSON.stringify({
        ...
        files: task
          ? task.files.map((file) =>
              file.encoding ? { ...file, content: '[Image attached directly to this session]' } : file,
            )
          : files,
  ...
  private launch(s: PlanSession, prompt: string) {
    ...
    job.done = this.client
      .runTurn({
        ...
        prompt,
        images: s.task?.files.flatMap((file, index) =>
          file.encoding === 'base64' ? [attachmentPath(s.cwd, file.name, index)] : [],
        ),
        signal: job.abort.signal,
        onStarted: async (ids) => {
          Object.assign(job, ids);
