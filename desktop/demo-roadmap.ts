...
export async function roadmapTask(
  ...
  const files = task.launchId
    ? await Promise.all(
        ['product-brief.md', 'assumptions.csv', 'customers.csv', 'forecast-contract.md'].map(
          async (name) => ({
            name,
            mediaType: name.endsWith('.csv') ? 'text/csv' : 'text/markdown',
  ...
  return {
    ...
    files,
    ...(task.launchId
      ? { project: 'little-office' as const, launchId: task.launchId, launchStep: task.launchStep }
      : {}),
