...
export async function verifyLaunchForecast(
  workspace: string,
  task: LocalTaskInput,
  fixture: string,
  ...
  if (rows.shift()?.join(',') !== columns.join(','))
    throw new Error(`forecast.csv must use columns ${columns.join(',')}.`);
  const contract = JSON.parse(await readFile(path.join(fixture, 'forecast-contract.json'), 'utf8')) as {
  ...
  for (let i = 0; i < rows.length; i++) {
    if (rows[i].length !== columns.length || rows[i][0] !== expected[i][0])
      throw new Error('Forecast months or columns do not match the launch contract.');
    for (let j = 1; j < columns.length; j++) {
      const value = Number(rows[i][j]);
      if (!rows[i][j] || !Number.isFinite(value) || Math.abs(value - Number(expected[i][j])) > 0.005)
        throw new Error(
          `Check ${rows[i][0]} ${columns[j]} against the supplied forecast formula and assumptions.`,
        );
    }
  }
  if (task.launchStep === 'revision') {
    ...
    if (original.content.trim() === content.trim())
      throw new Error('The competitor scenario did not revise the original forecast.');
  }
  return {
    id: randomUUID(),
    title:
      task.launchStep === 'revision' ? 'Revised forecast · verified CSV' : 'Baseline forecast · verified CSV',
    kind: 'report',
    filePath,
    content,
    simulated: false,
