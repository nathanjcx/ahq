...
Do not overwrite an existing `demo` branch. Deploy the completed web migration branch after its code, Convex functions, and this configuration have been reviewed. Do not run a seed function or import demo data into the production Convex deployment.
...
2. Enable Organizations. Require organization membership for a company workspace, then create the first organization or invite the operators who will test it. Configure roles so the intended admin can publish employees.
...
npx convex env set --prod MCP_TOOL_REGISTRY_JSON '{"linear":["list_issues","get_issue"]}'
```

The example commands are illustrative. Set the complete URL and tool registries from `.env.example` before connecting providers. `MCP_SERVER_URLS_JSON` is an exact per-provider URL admission list. `MCP_TOOL_REGISTRY_JSON` is an exact per-provider list of discovered, reviewed tool names that can be referenced by a published employee capability. A tool still needs a connection grant, employee capability, policy, and resource scope at runtime. Generate `AHQ_SERVICE_SECRET` and `CREDENTIAL_ENCRYPTION_KEY` independently. For the encryption key, use 32 random bytes encoded as base64, for example `openssl rand -base64 32`. The web and gateway validate this key as an AES-256-GCM key.
...
| --------- | ----------------- | ----------- | ------------------------------------------------------ |
| `web`     | `npm start`       | `/`         | Add the application domain and HTTPS                   |
...
The exact names come from the current web, worker, gateway, and Convex code. Add the common values to each Railway service that needs them. The simplest safe setup is to share the non-public runtime configuration with all three services, then seal secrets in Railway. Do not put any of these values in `NEXT_PUBLIC_*` variables.
...
| `CREDENTIAL_ENCRYPTION_KEY`         | web, gateway         | no     | yes for integrations   | 32 base64 encoded random bytes                                           |
...
| `S3_ENDPOINT`                       | worker               | no     | yes for archived files | S3-compatible endpoint                                                   |
| `S3_REGION`                         | worker               | no     | no                     | Defaults to `auto`                                                       |
| `S3_BUCKET`                         | worker               | no     | yes for archived files | Artifact bucket                                                          |
| `S3_ACCESS_KEY_ID`                  | worker               | no     | yes for archived files | Bucket access key                                                        |
| `S3_SECRET_ACCESS_KEY`              | worker               | no     | yes for archived files | Bucket secret                                                            |
...
| `MCP_APPROVED_HOSTS`                | web, gateway         | no     | no                     | Additional approved MCP hostnames                                        |
| `MCP_TOOL_POLICIES_JSON`            | web, gateway         | no     | no                     | Explicit tool policy overrides                                           |
| `MCP_OAUTH_CONFIG_JSON`             | web                  | no     | for OAuth              | Provider OAuth clients                                                   |
...
| `TASK_RESERVED_COST_USD`            | worker               | no     | no                     | Global reservation override                                              |
| `TASK_RESERVED_COST_USD_LUNA`       | worker               | no     | no                     | Luna reservation override                                                |
| `TASK_RESERVED_COST_USD_TERRA`      | worker               | no     | no                     | Terra reservation override                                               |
| `TASK_RESERVED_COST_USD_SOL`        | worker               | no     | no                     | Sol reservation override                                                 |
| `TASK_RESERVED_COST_USD_ASTRA`      | worker               | no     | no                     | Astra reservation override                                               |

Railway supplies `PORT`. Leave it unset so each process binds to Railway's assigned port. Set `MCP_SERVER_URLS_JSON` in Convex even when the same value exists in Railway. Convex rejects a connection when its URL is absent from the exact provider array. `MCP_APPROVED_HOSTS` accepts hostnames for approved custom ServiceNow endpoints or another explicitly reviewed server. The code still requires HTTPS and blocks IP addresses, credentials, fragments, and non-standard ports.
...
Connecting an MCP server does not create an inbox subscription. The current version supports on-demand provider access and an explicit webhook relay. For each push connection, generate a random secret and add it to the web variable like this:
