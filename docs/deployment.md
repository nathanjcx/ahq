...
Still to set by hand in the WorkOS dashboard: the JWT template with `name` and `email` claims (section 1), and the sign-out redirect `https://app.trystaff.ai/`. Optional: the three `VAPID_*` variables for web push, and Railway's draining seconds on `worker` (see operations.md).
