...
WorkOS production also has Magic Auth, Email + Password, and Google OAuth enabled, the JWT template with `name` and `email` saved, and the sign-out redirect `https://app.trystaff.ai/`; these were set through the WorkOS MCP server with a dashboard OAuth token (client and refresh token under `~/.config/trystaff/mcp/`, driven by `mcp.py` there). Optional: the three `VAPID_*` variables for web push, and Railway's draining seconds on `worker` (see operations.md).
