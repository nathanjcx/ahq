...
An integration is a configured connection to an approved server, with an authenticated provider identity and verified capabilities. The six initial families are Salesforce, Linear, Slack, Google Workspace, GitHub, and ServiceNow. Google Workspace appeared twice in the request; this plan treats it as one family with multiple product connections.
...
| Salesforce       | Official hosted platform MCP servers, including configurable server tool sets. [Overview](https://developer.salesforce.com/docs/platform/hosted-mcp-servers/guide/hosted-mcp-servers-overview.html)                                            | Customer-specific object/field allowlists. Verify org entitlement, server configuration, read-before-write, and a safe correction path. Developer tooling MCP is not a substitute for CRM access.      |
| ServiceNow       | Official MCP connection documentation is instance-specific; fetched content did not establish a complete external-client onboarding contract. [Docs](https://www.servicenow.com/docs/r/intelligent-experiences/connect-mcp-server-client.html) | Treat deployment, entitlement, authentication, exposed tools, and incident/change permissions as an integration spike. Do not claim a universal ready-to-connect server.                               |
...
| ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| Salesforce field update                         | Restore only the agent-changed fields if their current values/version still match; append an explanation where supported. | A workflow may already have fired. Restoring fields cannot erase downstream effects. |
| Salesforce record creation                      | Mark cancelled, superseded, or erroneous using an approved object-specific operation.                                     | Do not delete the record merely to hide the mistake.                                 |
| Linear/GitHub/ServiceNow status or field change | Apply a conditional compensating update and link the original action.                                                     | Later human changes, workflow rules, and missing update tools can block correction.  |
...
7. Add the remaining provider families using the same admission checklist. Ship only verified capabilities. Resolve Slack distribution, Google preview access, and ServiceNow onboarding before calling those connections production-ready.
...
- Launch access to Slack, Google preview, and ServiceNow, plus whether any real desktop data needs import.
