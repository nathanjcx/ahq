...
export type TaskStatus = 'queued' | 'running' | 'awaiting_approval' | 'completed' | 'failed' | 'cancelled' | 'uncertain';
export type CorrectionKind = 'supported' | 'partial' | 'manual' | 'irreversible' | 'unknown';
export interface Capability { provider: ProviderId; tools: string[]; optional: boolean }
export interface Listing {
  id: string; versionId: string; name: string; role: string; description: string;
  category: string; strengths: string[]; limitations: string[]; capabilities: Capability[];
  model: ModelId; color: string; media: {url: string; type: 'image' | 'video'; alt: string}[];
  publishedAt: number;
}
...
export interface Connection { id: string; provider: ProviderId; name: string; account: string; status: 'connected' | 'disconnected' | 'degraded' | 'revoked'; tools: string[]; allowedTools: string[]; resourceScope: string; inboxResources: string[]; lastCheckedAt?: number; inboxMode: 'push' | 'on-demand' | 'unsupported'; error?: string }
...
export interface InboxItem { id: string; provider: ProviderId; title: string; preview: string; sourceUrl?: string; createdAt: number; status: 'unread' | 'read' | 'assigned'; taskId?: string }
export interface Artifact { id: string; taskId: string; name: string; mediaType: string; size: number; createdAt: number }
export interface ProviderReadiness { provider: ProviderId; enabledUrls: string[]; reviewedTools: number }
