...
export function triagePrompt(source: SourceItem, state: Snapshot): string {
  const words = new Set(`${source.title} ${source.content}`.toLowerCase().match(/[a-z0-9][a-z0-9-]{3,}/g) || []);
  const related = state.sources.filter((item) => item.id !== source.id).map((item) => {
    const text = `${item.title} ${item.content}`.toLowerCase();
    return { item, score: [...words].filter((word) => text.includes(word)).length };
  }).sort((a, b) => b.score - a.score || b.item.timestamp - a.item.timestamp).slice(0, 32).map(({ item }) => item);
