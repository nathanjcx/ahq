...
export default function DemoPanel({
  ...
  const matchingSessions = savedSessions.filter(
    (session) =>
      !query ||
      `${name(session)} ${session.status} ${session.output?.content ?? ''}`.toLowerCase().includes(query),
  );
  ...
  return (
    <>
      ...
      <aside className="demo-dock" aria-label="Live work and session history" hidden={!visible}>
