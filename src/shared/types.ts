...
export interface SourceItem {
  ...
  scenario?: Scenario; disposition?: 'pending' | 'ignored' | 'attached' | 'work'; reason?: string;
}
...
export interface Snapshot {
  ...
  calendar: CalendarEvent[]; auth: AuthState;
  settings: { mode: 'demo' | 'live'; reducedMotion: boolean; sound: boolean; model: string };
  demo: { playing: boolean; nextIndex: number; speed: number };
}
export type Command =
  | { type: 'snapshot' }
