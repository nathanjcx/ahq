    ...
    function make(kind: "wood" | "fabric" | "plaster") {
      ...
      for (let y = 0; y < width; y++)
        for (let x = 0; x < width; x++) {
          ...
          const grain =
            Math.sin(y * 0.73 + Math.sin(x * 0.034) * 2.8) * 5 +
            Math.sin(y * 2.1 + x * 0.021) * 2;
  ...
  return (
    <group>
      ...
      {[1, 3].map((index) => (
        <group
          key={index}
          position={[desks[index][0], 0.985, desks[index][2] - 0.34]}
        >
          <Round
            p={[0, 0.022, 0]}
            s={[0.37, 0.04, 0.25]}
...
function activityFor(employee: Employee) {
  ...
  return employee.status === "review" ||
    /whiteboard|discuss|collaborat/.test(task)
    ? "discussion"
  ...
  return activity === "discussion"
    ...
    : activity === "reading"
      ...
      : activity === "lounge"
        ...
        : [
            desks[index % desks.length][0],
            0.105,
            desks[index % desks.length][2] + 1,
          ];
}
...
function HandoffRoute({
  ...
  const curve = useMemo(() => {
    ...
    if (start[0] < 0) {
      points.push(
        [start[0], 0.105, start[2] + 0.54],
        [0.15, 0.105, start[2] + 0.54],
        [0.15, 0.105, -1.36],
      );
    }
    if (end[0] > 1.45 && end[2] < -0.6)
      points.push([2.16, 0.105, -1.36], [2.16, 0.105, -4.94]);
    points.push(end);
  ...
  useFrame((_, delta) => {
    ...
    if (halo.current)
      halo.current.scale.setScalar(1 + Math.sin(clock.current * 2.4) * 0.1);
  });
  return (
    <group>
      <Line
...
function WorkflowHandoffs({ team, motion, timeline, selected }: OfficeProps) {
  const reviewer = team.findIndex(
    (person) => activityFor(person) === "discussion",
  );
  const researcher = team.findIndex(
    (person) => activityFor(person) === "research",
  );
  const creative = team.findIndex(
    (person, i) => i !== reviewer && activityFor(person) === "discussion",
  );
  ...
  return (
    <group>
      <HandoffRoute
        ...
        highlighted={
          !selected ||
          selected === team[researcher].id ||
          selected === team[destination].id
        }
      />
      {creative >= 0 && (
        <HandoffRoute
...
function Framing({ zoom, angle }: Pick<OfficeProps, "zoom" | "angle">) {
  ...
  useLayoutEffect(() => {
    ...
    camera.zoom =
      Math.min(safeWidth / w, safeHeight / h) * Math.max(0.5, zoom / 37);
  ...
  return (
    <group position={p} rotation={[0, rotation, 0]}>
      ...
      {[-1, 1].map((side) => (
        <group key={side}>
          <Box
            p={[side * 0.32, 0.63, 0.04]}
            s={[0.035, 0.24, 0.035]}
            color="#4b5855"
          />
          <Round
            p={[side * 0.32, 0.74, 0.04]}
            s={[0.07, 0.065, 0.34]}
            color="#4b5855"
            radius={0.022}
          />
    ...
    <group>
      ...
      {[-1, 1].map((side) => (
        <group key={side}>
          ...
          {index % 3 === 0 && (
            <Box
              p={[side * 0.087, headY + 0.012, 0.175]}
              s={[0.13, 0.075, 0.018]}
              color="#454d46"
            />
          )}
...
function EmployeeAvatar({
  ...
  const activity =
    ...
    /whiteboard|discuss|collaborat/.test(classification)
      ? "discussion"
      : /cabinet|librar|reading|read documents|dependencies|resources/.test(
            classification,
          )
        ? "reading"
        : employee.status === "ready"
    ...
    activity === "discussion"
      ...
      : activity === "reading"
        ...
        : activity === "lounge"
          ...
          : [desk[0], 0, desk[2] + 1];
  const isSeated =
    !walking && (activity === "research" || activity === "lounge");
  const idleYaw =
    activity === "reading" ||
    activity === "discussion" ||
    activity === "research" ||
  ...
  const route = useMemo<Point[]>(() => {
    const corridorX =
      employee.position &&
      employee.position[0] > -0.7 &&
      employee.position[0] < 0.75
        ? employee.position[0]
        : 0.25;
    ...
    return [
      home,
      [home[0], 0, home[2] + 0.52],
      [corridorX, 0, home[2] + 0.52],
      [corridorX, 0, -0.9],
    ];
  ...
  useFrame((_, delta) => {
    ...
    if (moving) {
      ...
      const lengths = route
        .slice(1)
        .map((p, i) => Math.hypot(p[0] - route[i][0], p[2] - route[i][2]));
      const distance = lengths.reduce((sum, n) => sum + n, 0) * outAndBack;
      let consumed = 0;
      let seg = 0;
      while (seg < lengths.length - 1 && consumed + lengths[seg] < distance)
        consumed += lengths[seg++];
      const start = route[seg];
      const end = route[seg + 1];
      const fraction = Math.min(
        1,
        Math.max(0, (distance - consumed) / Math.max(0.001, lengths[seg])),
      );
      ...
      const targetYaw = Math.atan2(
        (end[0] - start[0]) * direction,
        (end[2] - start[2]) * direction,
      );
      ...
      group.current.rotation.y =
        idleYaw +
        (activity === "discussion"
          ? Math.sin(elapsed.current * 0.8) * 0.18
          : 0);
    }
    if (figure.current)
      figure.current.rotation.z =
        !moving && activity === "discussion"
          ? Math.sin(elapsed.current * 1.8) * 0.026
          : 0;
    ...
    <group ref={group} position={home} rotation={[0, idleYaw, 0]}>
      ...
      <Html
        ...
        position={[
          0,
          (isSeated ? 1.89 : 2.18) +
            (activity === "discussion" ? (index % 2) * 0.28 : 0),
          0,
        ]}
