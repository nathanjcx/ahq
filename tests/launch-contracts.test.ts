...
const milestones = () =>
  stages.map((launchStep, index) => ({
    key: launchStep,
    launchStep,
    taskKind: ['product', 'meeting', 'report'][index],
    title: `Little Office ${launchStep}`,
    description: 'Create the actual deliverable using supplied launch source files.',
    ownerId: '',
    dayOffset: 1,
    dependencies: [],
    definitionOfDone: 'Real files pass the local verification.',
    nextStep: 'Read the launch brief and input files.',
  }));
test('real launch planner contract produces three typed parallel tasks and rejects dropped responsibilities', async () => {
  ...
  const plan = await generateRoadmap(async (prompt) => {
    assert.match(prompt, /LITTLE OFFICE LAUNCH/);
    return JSON.stringify({ milestones: milestones() });
  }, input);
  assert.deepEqual(
    plan.map((task) => task.launchStep),
    stages,
  );
  assert(plan.every((task) => task.launchId === 'launch-a' && task.dependencies.length === 0));
  const bad = milestones();
  bad[2].launchStep = 'product';
  await assert.rejects(
    generateRoadmap(async () => JSON.stringify({ milestones: bad }), input),
    /independent product/,
  );
});
test('screenshots require canonical base64 PNG or JPEG, text stays bounded', () => {
  ...
  assert(
    DemoTriggerSchema.safeParse({
      kind: 'slack',
      attachments: [{ name: 'bug.png', mediaType: 'image/png', encoding: 'base64', content: png }],
    }).success,
  );
  assert(
    !DemoTriggerSchema.safeParse({
      kind: 'slack',
      attachments: [{ name: 'bug.png', mediaType: 'image/png', encoding: 'base64', content: 'hello' }],
    }).success,
  );
  assert(
    !DemoTriggerSchema.safeParse({
      kind: 'email',
      attachments: [{ name: 'a.txt', mediaType: 'text/plain', content: 'a'.repeat(64001) }],
    }).success,
  );
});
test('forecast checks numerical output and stale renders cannot undo a restored checkpoint', async () => {
  ...
  try {
    ...
    await writeFile(
      path.join(directory, 'forecast-contract.json'),
      JSON.stringify({ baseline: [row], revised: [row] }),
    );
    const csv =
